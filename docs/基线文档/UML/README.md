...
| 图 | 文件 | 说明 |
|---|---|---|
| 用例图 | `用例图.puml` / `用例图.png` | 4 类参与者 × 33 个用例，覆盖认证、班级学生、奶品配额、订单支付、配送、营养、可视化、系统管理 9 个模块，与 `docs/基线文档/接口文档.md` 逐条对应 |
| 类图 | `类图.puml` / `类图.png` | 23 个实体类（22 个 `module/**/entity` 实体 + 1 个过程层台账实体 `ProcessTransitionLog`）按 6 个领域包组织，字段取自代码实体，标注主要关联关系 |
| 部署图 | `部署图.puml` / `部署图.png` | 家长小程序 / 管理端浏览器 → Nginx(80) → Spring Boot(8090) → MySQL(3306)，含 JWT 过滤器、分层、定时任务、微信支付模拟链路 |

> **图片待重新渲染（2026-09-19）**：本轮清理删除了续订模块（实体/接口/表/页面）并新增了过程迁移台账，
> 三份 `*.puml` 源文件已同步，但仓库内的 `*.png` 仍是 2026-09-15 的旧渲染版本。
> 需要按下方「修改方式」重新渲染后再用于论文与答辩材料。
