 ...
 * 父订单聚合为已完成。这两次推进都是普通的数据库写入，一旦进程中断或异常，
 * 父状态就会与子任务集合不一致，系统进入一个“自洽但错误”的状态。</p>
 *
 * <p>本实验人为制造这两种漂移，再运行对账补偿任务，验证系统能
 * 「发现不一致 → 补偿 → 回到合法状态」，且补偿本身幂等可重复执行。</p>
 */
...
class Experiment4ProcessReconcileTest extends ExperimentSupport {
    ...
    @Test
    @DisplayName("人为制造父状态漂移，对账任务补偿回合法状态且幂等")
    void reconcileRepairsParentStatusDrift() throws Exception {
        ...
        int repaired1 = orderInfoService.reconcileOrderAggregation(BATCH_LIMIT);
        ...
        int repaired2 = orderInfoService.reconcileOrderAggregation(BATCH_LIMIT);
        ...
        orderInfoService.reconcileOrderAggregation(BATCH_LIMIT);
