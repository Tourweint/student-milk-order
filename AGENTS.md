...
- **重要功能先写设计方案**（`docs/设计方案/YYYY-MM-DD-描述-设计方案.md`）：记录设计取舍、备选与被明确排除的做法。
  实现完成后**不要删除设计方案**，而是把内容**下沉**到长期文档（接口 → `docs/基线文档/接口文档.md`、业务规则 →
  `docs/研发规范/项目开发规范.md`、机制与判定口径 → `docs/基线文档/可靠性设计.md`、实测 → `docs/实验/`、
  落地记录 → `docs/变更记录/`），并在方案文首补「状态 + 下沉落点表 + 冲突以长期文档为准」。
  分工：`docs/基线文档/` 是"现在是什么规则"（长期、不带日期）；`docs/设计方案/` 与 `docs/变更记录/` 是过程文档
  （设计期 / 落地期，均带日期）。**不要因为"内容已下沉"就删掉设计方案**——它是工程过程的证据。
- **图统一用 draw.io,不再用 PlantUML/Java 渲染**(2026-09-21 起):架构、流程、状态机、部署、时序等图件的新增与修改一律产出 `*.drawio` 源文件(mxGraph XML、UTF-8、容器分组、统一配色、正交连线),放入 `docs/基线文档/UML/`;主会话可直接使用 drawio MCP 工具(`drawio__open_drawio_xml` 预览、`drawio__search_shapes` 查官方样式、`drawio__list_pages` / `drawio__get_page` / `drawio__set_page` 分页读写)。文档内嵌的小图可继续用 Mermaid(GitHub 直接渲染)。风格与流程约定见 `docs/基线文档/UML/README.md`。
...
- **退款与毕业清算（退款域）**：退款单是**挂在订单旁侧的资金过程**，订单状态零新增（作废期次后由既有聚合出口收敛）。
  ...
  `order_info.contract_total_boxes` **支付时快照**（平移/补送/摊平都会改任务行，实时求和必漂移），
  分子扣掉补送的免费盒，套餐按累计制均摊、零散按明细单价精确；③ 可退期次判据与 `INV_TASK_COMPENSATION` 同判据，
  **退款/平移作废的任务不得把签收记录置为拒收**（否则会被下一次退款当"缺货取消"再退一遍），
  已退订(5) 订单整体排除（避免与退订全额退款重复）。申请幂等靠生成列 `active_order_id` + `uk_refund_active`；
  毕业清算走专属取消路径（不复用 `cancelTimeoutOrder`）。业务规则见 `docs/研发规范/项目开发规范.md` §5.7，
  机制见 `docs/基线文档/可靠性设计.md` §十五，接口见 `docs/基线文档/接口文档.md` §10。
